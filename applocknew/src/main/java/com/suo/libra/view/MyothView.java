...
public class MyothView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public MyothView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public MyothView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public MyothView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    boolean startCamera() {
        if (!SharPre.fetchIntruder()) return false;
        try {
                    ...
                    camera.setOneShotPreviewCallback(new Camera.PreviewCallback() {
                        ...
                        public void onPreviewFrame(final byte[] data, final Camera cam) {
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        if (soundPool != null&& Preference.isShutterSoundEnabled()) {
                                        ...
                                        if (InvadeApi.addIntruder(bitmap, pkgName)) {
                                            SharPre.setHasIntruder(true);
                                        }
