			...
			public boolean shouldOverrideUrlLoading(WebView view, String url) {
				if (url.startsWith(GOOGLE_MARKET)) {
					try {
						...
						if (MoreApp.this.getPackageManager().queryIntentActivities(i, PackageManager.MATCH_DEFAULT_ONLY).size() > 0) {
							MoreApp.this.startActivity(i);
						} else {
							MoreApp.this.startActivity(new Intent(Intent.ACTION_VIEW,
...
//				}
		...
		wv.addJavascriptInterface(new JSObject(this), JSObject.NAME);
...
//			this.checkVersion();
			MoreApp.this.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					wv.stopLoading();
					wv.loadUrl(url);
...
//	}
	...
	public static void show(String url) {
		_context.startActivity(new Intent(_context, MoreApp.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra(Intent.EXTRA_TEXT, url));
	}
	
	public static void show(Context context, String url) {
		if(_context == null)
			_context = context;
		_context.getApplicationContext().startActivity(new Intent(_context, MoreApp.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK).putExtra(Intent.EXTRA_TEXT, url));
	}
